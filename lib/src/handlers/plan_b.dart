...
class PlanB extends HandlerInterface {
  ...
  Future<void> _setupTransport(
      {required DtlsRole localDtlsRole, SdpObject? localSdpObject}) async {
  ...
  SctpCapabilities getNativeSctpCapabilities() {
    ...
    return SctpCapabilities(
      numStreams:
          NumSctpStreams(mis: SCTP_NUM_STREAMS.MIS, os: SCTP_NUM_STREAMS.OS),
    );
  }
  ...
  Future<void> stopSending(String localId) async {
    ...
    RTCRtpSender? sender =
        senders.firstWhereOrNull((e) => e.senderId == track.id);
    if (sender == null) {
